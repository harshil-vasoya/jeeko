...
export default function TillerMachinesPage() {
  const tillerMachines = getProductsByCategory("tiller machine")
    ...
    <section className="py-12 md:py-16 lg:py-20">
      <div className="container">
        <div className="flex flex-wrap items-center gap-2 mb-8">
          ...
          <ChevronRight className="h-4 w-4 text-gray-400" />
          <span className="text-sm text-gray-900 font-medium">Tiller Machines</span>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {tillerMachines.map((tiller, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
              <div className="h-56 relative">
                <Image
                  src={tiller.imageSrc || "/placeholder.svg?height=200&width=200&query=tiller+machine"}
                  alt={tiller.name}
                  fill
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-xl font-bold">{tiller.name}</h3>
                  <span className="bg-[#8dc63f]/20 text-[#8dc63f] text-xs font-medium px-2 py-1 rounded">
                    Power Weeder
                  </span>
                </div>
                <p className="text-gray-600 mb-4">{tiller.description}</p>
                <div className="mb-4">
                  <h4 className="font-medium mb-2">Key Features:</h4>
                  <ul className="space-y-1">
                    {tiller.features.slice(0, 4).map((feature, idx) => (
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button asChild className="bg-brand-secondary hover:bg-brand-secondary/90">
                    <Link href={`/products/tiller-machines/${tiller.slug}`}>View Details</Link>
                  </Button>
