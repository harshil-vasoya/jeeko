...
import { tillerMachines } from "@/lib/products"
import { notFound } from "next/navigation"

export default function TillerMachineDetailPage({ params }: { params: { slug: string } }) {
  const slug = params.slug
  const product = tillerMachines[slug as keyof typeof tillerMachines]
  ...
  const relatedProducts = Object.values(tillerMachines)
    .filter((tiller) => tiller.slug !== slug)
    .slice(0, 2)
    ...
    <section className="py-12 md:py-16 lg:py-20">
      <div className="container">
        <div className="flex flex-wrap items-center gap-2 mb-8">
          ...
          <ChevronRight className="h-4 w-4 text-gray-400" />
          <Link href="/products/tiller-machines" className="text-sm text-gray-500 hover:text-gray-900">
            Tiller Machines
          </Link>
          <ChevronRight className="h-4 w-4 text-gray-400" />
        ...
        <div className="grid md:grid-cols-2 gap-12 mb-16">
          <div className="bg-white p-8 rounded-lg shadow-md">
            <div className="relative h-[400px] mb-4">
              <Image
                src={product.imageSrc || "/placeholder.svg?height=400&width=400&query=tiller+machine"}
                alt={product.name}
                fill
            ...
            <div className="grid grid-cols-4 gap-2">
              <button className="border rounded-md p-2 hover:border-green-500 transition-colors">
                <div className="relative h-16">
                  <Image
                    src={product.imageSrc || "/placeholder.svg?height=100&width=100&query=tiller+machine+angle1"}
                    alt="Front view"
                    fill
                    className="object-contain"
                  />
              ...
              <button className="border rounded-md p-2 hover:border-green-500 transition-colors">
                <div className="relative h-16">
                  <Image
                    src={product.imageSrc || "/placeholder.svg?height=100&width=100&query=tiller+machine+angle2"}
                    alt="Side view"
                    fill
                    className="object-contain"
                  />
              ...
              <button className="border rounded-md p-2 hover:border-green-500 transition-colors">
                <div className="relative h-16">
                  <Image
                    src={product.imageSrc || "/placeholder.svg?height=100&width=100&query=tiller+machine+angle3"}
                    alt="Top view"
                    fill
                    className="object-contain"
                  />
              ...
              <button className="border rounded-md p-2 hover:border-green-500 transition-colors">
                <div className="relative h-16">
                  <Image
                    src={product.imageSrc || "/placeholder.svg?height=100&width=100&query=tiller+machine+in+use"}
                    alt="In use"
                    fill
                    className="object-contain"
                  />
                ...
                {product.accessories.map((accessory, index) => (
                  <div key={index} className="flex flex-col items-center text-center">
                    <div className="h-24 w-24 relative mb-2">
                      <Image
                        src={accessory.image || "/placeholder.svg?height=100&width=100&query=accessory"}
                        alt={accessory.name}
                        fill
                        className="object-contain"
                      />
            ...
            {relatedProducts.map((relatedProduct, index) => (
              ...
              >
                <div className="h-48 relative">
                  <Image
                    src={relatedProduct.imageSrc || "/placeholder.svg?height=200&width=200&query=tiller+machine"}
                ...
                <div className="p-4 border-t">
                  ...
                  <Link
                    href={`/products/tiller-machines/${relatedProduct.slug}`}
